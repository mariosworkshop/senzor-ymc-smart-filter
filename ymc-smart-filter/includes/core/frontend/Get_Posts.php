...
class Get_Posts {
	...
	public function get_filter_posts() {
		...
		$default_terms = ! empty( $clean_data['default_terms'] ) ? explode(',', $clean_data['default_terms']) : false; //extract default terms from posted data
			...
			foreach ($taxonomy as $tax) :
				...
				if( !empty($term_id) ) :
					...
					$tax_qry[] = [
						...
						'field' => 'term_id',
						'terms' => $term_id,
						'operator' => (($clean_data['posts_selected'] == "all") && ($default_terms != $terms) || count($terms) == count($terms_selected)) ? 'OR' : 'AND' //set inner relations to AND (when filter items is selected) or OR (when all button is clicked or all filter items is checked off)
					];
