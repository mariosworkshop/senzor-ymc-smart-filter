...
class Shortcode {
	...
	public function ymc_filter_apply( $atts )
	{
		...
		$atts = shortcode_atts( [
			'id' => '',
			'default_term' => '', //new attribute for set up default term
			'filter_multiple_settings' => '', //new attribute for set up mutliple taxonomy for each filter
		], $atts );

		$id = (int) $atts['id'];
		
		$default_term = (string) $atts['default_term']; //converting default term from array of attributes to default_term attribute
		...
		$ymc_filter_multiple_settings = (string) $atts['filter_multiple_settings']; //converting multiple taxonomy filter settings from array of attributes to string of multiple filter settings
		...
		if ( !empty($id) && $ymc_post_type === 'ymc_filters' && $post_status === 'publish' )
		{
			    ...
			    $ymc_filter_layout !== 'alphabetical-layout' )
			{
				...
				/*   Add functionality for custom default term in filter for each site   */
				
						foreach($ymc_terms_options as &$ymc_terms_option){
							if($ymc_terms_option['name'] === $default_term){
								$ymc_terms_option['default'] = 'checked';
							}
						}

						unset($ymc_terms_option);
